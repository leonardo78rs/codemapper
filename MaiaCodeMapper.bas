       ...
       If Len(Trim(cFile)) > 0 Then
        ...
        Else
               ...
               If nFonteEmBranco > 3 Then
                  nfontes = 800         'Força a sair do loop
               End If
    ...
    If cBusca <> Empty Then   ' PERGUNTAR SE NAO É A PRIMARIA
       ...
       If Mid(cBusca, 1, Len(cBusca) - 2) <> wOcorr.Cells(2, 2) And _
          Trim(Mid(cBusca, 1, LenB(cBusca) - 2)) <> Trim(wOcorr.Cells(2, 2)) Then               ' Nao buscar a própria palavra Ocorrencias!B2
            ...
            ' Agrupa funções  (False - não exibe animação no secundário)
            GroupFunc (False)
...
For nLimpa = 13 To 500
    ...
    If nEmBranco > 3 Then
       nLimpa = 500         'Força a sair do loop
    End If
...
MsgBox ("- Não procura em linhas comentadas com // , e */ /* quando estiver na mesma linha" + Chr(10) + Chr(10) + _
        "- Porém ainda não entende quando for BLOCO DE CÓDIGO comentado com  */ e /* em linhas diferentes " + Chr(10) + Chr(10) + _
        "- Não testei ainda PL/SQL, mas deve funcionar " + Chr(10) + Chr(10) + _
