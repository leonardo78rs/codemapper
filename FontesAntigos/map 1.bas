...
For nFontes = 1 To 574
    wSource.Cells(nFontes, 3) = ""
    If wSource.Cells(nFontes, 2) = "S" Then
       cFile = wSource.Cells(nFontes, 1)
       na = ImportTxtFile(cPath, cFile, cBusca, 1, na)
       wSource.Cells(nFontes, 3) = "Verif"
    End If
Next
...
For lin1 = 1 To 600    'criar variavel ou celula para total
    ...
    If nini = 0 Then
       nini = InStr(1, wPlan2.Cells(lin1, 5), "procedure", vbTextCompare) + 9
    End If
...
For lin1 = 2 To 900      'criar variavel ou celula para total
    lin3 = lin2
    Do While True
        
        If wPlan2.Cells(lin2, 1) = "" Then
           lin2 = lin2 + 1
           Exit Do
        End If
        
        If wOcorr.Cells(lin1, 1) <> wPlan2.Cells(lin2, 1) Then
           lin2 = lin2 + 1
        End If
    
        If wOcorr.Cells(lin1, 2) > wPlan2.Cells(lin2, 2) And _
           wOcorr.Cells(lin1, 2) < wPlan2.Cells(lin2 + 1, 2) Then
           
           wOcorr.Cells(lin1, 4) = wPlan2.Cells(lin2, 4)
           lin2 = lin3
           Exit Do
        End If
